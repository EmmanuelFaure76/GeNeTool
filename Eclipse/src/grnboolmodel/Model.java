...
public class Model {
  ...
  public Model(GRNBoolModel p, int number,String date){
    this.p = p;
      p.mm.addMessage("Create model "+this.number+ " at " + date);
      this.Name="default";
      this.reset();
      this.number=number;
      this.date=date; 
  ...
  public Model(Model model,int number,String Name,String date){
    this.p = model.p;
    p.mm.addMessage("Duplicate from model "+model.number+ " at " + date);
    this.number=number;
    this.date=date;
    this.Name=Name;
  ...
  public void reset(){ 
      Correspons=new int[p.maxStep]; for(int i=0;i<p.maxStep;i++)Correspons[i]=i;
      step=0;ActualStep=0;
  }
 ...
 public void delGene(Gene g){ 
  ...
  public ModelDomain getDomain(String name){
      for(int d=0;d<this.modelDomains.size();d++){
        ModelDomain md=this.getDomain(d);
        if(p.uf.equal(md.dom.Name,name)) return md;
      }
      return null;
  }
  ...
  //compute the Next step of the model
   public void Step(int s){
     p.mm.addMessage("Model " + this.Name +" -> Calcul Step " +(s+1) );
     for(int d=0;d<this.modelDomains.size();d++){ //For Each Domains
          ModelDomain md=this.getDomain(d);
          for(int i=0;i<p.gm.nbGene;i++){ //For Each Gene Verify the rules
              Gene gene=p.gm.getGene(i);
              p.dm.CommentsRule="";
              if(p.toDo==4 & md.Manual[i][s+1]) { } //If we are in restart mode, we don't change the value
              else{
                if(p.toDo!=4 && md.Manual[i][s] && s+1==ActualStep){  //If the box was Manual clicked and this is the last step
      ...
      while(loopCheck<5 && nbChagement>0){
       ...
       //   print(" Next Try "+loopCheck);
        ...
        if(nbChagement>0)   p.mm.addMessage("Model " + this.Name +" -> Try "+ loopCheck + " check -> "+nbChagement+" changement");
      }
      if(loopCheck==5) p.mm.addMessage("Model " + this.Name +" -> ERROR infinite loop at  " +this.Correspons + " "+p.rm.timeUnit);
      //BLUE MODULE
     for(int d=0;d<this.modelDomains.size();d++){ //For Each Domains
          ModelDomain md=this.getDomain(d);
          for(int i=0;i<p.gm.nbGene;i++){ //For Each Gene Verify if w have a blue modul
            Gene gene=p.gm.getGene(i);
   ...
   public void Step(){
     if(p.toDo!=4) this.Correspons[step+1]=this.Correspons[step]+1; //Else Restart Mode
